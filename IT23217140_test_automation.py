...
def _substring_at_token_boundaries(haystack: str, needle: str) -> bool:
    """
    True if needle occurs in haystack where both sides are non-alphanumeric boundaries.
    Prevents 'singlish' matching inside 'singlishinputtypescovered' and 'input' matching
    inside compound headers (common false positives for the Singlish input column).
    """
...
def _loose_header_match(norm_v: str, token_n: str) -> bool:
    """Original substring matching for expected/actual/status (e.g. 'expected' in 'expectedoutput')."""
    if not token_n:
        return False
    if norm_v == token_n:
        return True
    return token_n in norm_v or norm_v in token_n


def _is_metadata_input_header(norm_v: str, raw: str) -> bool:
    """Columns that look like taxonomy/rationale, not the Singlish cell to type."""
    raw_l = (raw or "").lower()
    if "types covered" in raw_l or "input types" in raw_l:
        return True
    if "evidence" in raw_l or "rational" in raw_l:
        return True
    if norm_v.endswith("typescovered") or "inputtypescovered" in norm_v:
        return True
    return False
...
def _find_column_index(header_values: list, requested_name: str | None, candidates: list[str]) -> int | None:
    ...
    def match(name: str) -> int | None:
        ...
        for i, v in indexed:
            ...
            if _loose_header_match(norm_v, n):
                return i
        return None
...
def _find_input_column_index(header_values: list, requested_name: str | None, candidates: list[str]) -> int | None:
    """Resolves the Singlish text column without matching taxonomy columns like 'Singlish input types covered'."""
    indexed = []
    for i, v in enumerate(header_values, start=1):
        if v is None:
            continue
    ...
    def input_match(name: str) -> int | None:
        n = _normalize_header(name)
        if not n:
            return None
        if n in norm_to_index:
        ...
        for i, v in indexed:
            norm_v = _normalize_header(v)
            raw = str(v)
            if _is_metadata_input_header(norm_v, raw):
                continue
            if norm_v == n:
                return i
            if _substring_at_token_boundaries(norm_v, n):
                return i
        return None

    if requested_name:
        found = input_match(requested_name)
        if found:
            return found
    ...
    # First column often has the phrase to type but no header label
    for i, v in enumerate(header_values, start=1):
...
def _clear_textarea(page, locator, attempts: int = 5):
...
def _read_output(is_chat: bool, output_locator) -> str:
    """
    TEXTAREA/INPUT: `.value` is authoritative (innerText is often empty).
    Other elements: prefer innerText/textContent.
    """
    try:
        snap = output_locator.evaluate(
            """(el) => {
              if (!el) return "";
...
def _click_transliterate(page, action_locator):
...
def _wait_translation_output(page, is_chat: bool, output_locator, prev_output: str, args) -> str:
    """
    After triggering translation, wait until Sinhala output becomes non-empty and updates vs prev_output.
    The UI keeps the previous Sinhala visible until the new response arrives, so we wait until the
    value changes or (first run) becomes non-empty.
    """
    actual_output = ""
    ...
    if not actual_output:
        # Cold proxies / headless: a few longer polls after primary retries
        for _ in range(12):
...
def _find_chat_locators(page, timeout_ms: int):
    ...
    while time.time() < deadline:
        ...
        action_ready = False
        try:
            action_ready = action.count() > 0 and action.is_visible()
        except Exception:
            action_ready = False
        try:
            ...
            if (
                input_by_ph.count() > 0
                and output_by_ph.count() > 0
                and input_by_ph.is_visible()
                and action_ready
            ):
        ...
        try:
            ...
            for i in range(count):
                ...
                if loc.is_visible():
                    visible.append(loc)
            # Guard against typing into unrelated sites (e.g., Google) by requiring
            # the Transliterate action to exist and be visible.
            if len(visible) >= 2 and action_ready:
                return visible[0], visible[1], action
            if count >= 2 and action_ready:
                input_loc = page.locator("textarea").nth(0)
                output_loc = page.locator("textarea").nth(1)
                if input_loc.count() > 0 and output_loc.count() > 0 and input_loc.is_visible():
...
def run_test():
    ...
    input_col_idx = _find_input_column_index(header_values, args.input_col, DEFAULT_INPUT_COLUMN_CANDIDATES)
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                _dismiss_overlays(page)
                # Snapshot current Sinhala before new input is typed.
                # Some builds transliterate live while typing; if we capture "prev"
                # after typing, we can accidentally treat the correct new text as stale.
                ...
                actual_output = ""
                if is_chat:
                    # Prefer waiting on the real translation HTTP response (more reliable than fixed sleeps).
                    try:
                        hook_timeout = min(25000, max(3000, int(args.timeout_ms)))
                        with page.expect_response(
                            lambda r: ("translate" in r.url.lower() or "chat" in r.url.lower())
                            and r.request.method == "POST",
                            timeout=hook_timeout,
                        ) as resp_info:
                            _click_transliterate(page, action_locator)
                        resp = resp_info.value
                        if resp.status >= 400:
                            print(f"Warning: translate HTTP {resp.status} for {resp.url}")
                    except Exception:
                        # Fallback: click without network hook (some builds may not expose POST reliably).
                        _click_transliterate(page, action_locator)

                    page.wait_for_timeout(max(0, int(args.wait_ms)))
                    actual_output = _wait_translation_output(page, is_chat, output_locator, prev_output, args)

                    if not actual_output:
                        # Second attempt: click again and wait longer (cold starts / proxy delays).
                        page.wait_for_timeout(500)
                        _click_transliterate(page, action_locator)
                        page.wait_for_timeout(max(0, int(args.wait_ms)))
                        actual_output = _wait_translation_output(page, is_chat, output_locator, prev_output, args)
                ...
                if not actual_output:
                    raise RuntimeError("No output text captured for this input.")
                ...
                if args.save_every and int(args.save_every) > 0 and processed % int(args.save_every) == 0:
                    try:
                        ...
                        print(f"Warning: Could not save Excel right now ({save_err}). Close the file if it's open and continue.")
